function filter = spatialFilterFunc(nx,ny,alpha,filterBorders,periodic)

% Implements the 10th order numerical filter by Gaitonde 1998 for spatial filtering

[filterStencilLHS, filterStencilRHS, filterDecenteredStencilLHS, filterDecenteredStencilRHS] = spatialFilterCoefficients(alpha,filterBorders);

LHSx = sparse(makeEachMatrix(filterStencilLHS,filterDecenteredStencilLHS,nx,periodic));
RHSx = sparse(makeEachMatrix(filterStencilRHS,filterDecenteredStencilRHS,nx,periodic));

LHSy = sparse(makeEachMatrix(filterStencilLHS,filterDecenteredStencilLHS,ny,periodic))';
RHSy = sparse(makeEachMatrix(filterStencilRHS,filterDecenteredStencilRHS,ny,periodic))';

filter = @(Y)(((LHSx\(RHSx*Y))*RHSy)/LHSy);
end
